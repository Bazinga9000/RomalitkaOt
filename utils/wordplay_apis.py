'''
Galactic wrote most of this code, not me.
They're insane. Some of this HTML is an utter clusterfuck
--Baz
'''
...
NUTRIMATIC_URL = 'https://nutrimatic.org/?q={content}&start=0&num={top_n}'
...
def onelook(pattern, top_n=100):
    ...
    return url, [(k['word'], k['score']) for k in json.loads(response.content)[:top_n]]


def nutrimatic(content, top_n=100):
    url = NUTRIMATIC_URL.format(content=urllib.parse.quote(content), top_n=top_n)
    response = requests.get(url)
...
def crossword(clue):
    ...
    def valid_class(c):
        ...
        return "even" in c or "odd" in c

    with open("soup.html", "w") as f:
        f.write(str(soup))

    print(soup.find_all(class_="wp-widget-content"))
    for tr in soup.find_all(class_=valid_class):
        print(tr)
        tds = tr.select('td')
...
def solve_cryptogram(message, top_n=8):
    ...
    def _threaded_qq_request(url, data, responses):
        ...
        print(json.loads(url_response.content))
